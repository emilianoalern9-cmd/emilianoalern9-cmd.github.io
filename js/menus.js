/* ═══════════════════════════════════════════════════════════
   menus.js — Constructores de menús, modales flotantes, CSV
   ═══════════════════════════════════════════════════════════ */
...
function generarMenuCategoria(cat, menuId) {
  ...
  opcionesVisibles.forEach(val => {
    ...
    cb.type     = "checkbox";
    cb.value    = val;
    cb.checked  = filtrosState[cat].includes(val);
    cb.disabled = solaUna;

    cb.addEventListener("change", () => {
      if (cb.checked) {
        if (!filtrosState[cat].includes(val)) filtrosState[cat].push(val);
      } else {
    ...
    const ts = document.createElement("span"); ts.className = "label-text"; ts.textContent = ` ${val}`;
    const cs = document.createElement("span"); cs.className = "count"; cs.textContent = `(${count})`;
    label.append(cb, ts, cs);
    menu.appendChild(label);
  });

  const bc = document.createElement("button");
  bc.textContent = "Limpiar esta categoría";
  bc.className = "limpiar-categoria";
...
function generarRevFilterMenu() {
  ...
  const ops = [{ id:"revisado", label:"Revisado" }, { id:"no_revisado", label:"No revisado" }];
  ...
  visibles.forEach(opt => {
    ...
    cb.type     = "checkbox";
    cb.value    = opt.id;
    cb.checked  = revFilterState.includes(opt.id);
    cb.disabled = solaUna;

    cb.addEventListener("change", () => {
      cb.checked
        ? (!revFilterState.includes(opt.id) && revFilterState.push(opt.id))
    ...
    const ts = document.createElement("span"); ts.className = "label-text"; ts.textContent = ` ${opt.label}`;
    const cs = document.createElement("span"); cs.className = "count"; cs.textContent = `(${count})`;
    label.append(cb, ts, cs);
    revFilterMenu.appendChild(label);
  });
...
function generarMenuOrden() {
  ...
  [...ordenMenu.children].forEach((b, i) => {
    b.onclick = () => {
      ...
      ordenActual = ORDEN_OPCIONES[i].v;
      currentPage = 1;
...
function generarMenuConfig() {
  ...
  const addBtn = (html, click) => {
    ...
    b.innerHTML = html;
    b.onclick = click;
    configMenu.appendChild(b);
    return b;
  };

  addBtn("🎲 Azar", () => {
    const f = aplicarFiltrosCompletos();
    if (f.length) { navegacionBloqueada = true; abrirDetalle(f[Math.floor(Math.random() * f.length)]); }
    closeAllMenus();
  });

  addBtn(`🔢 Ítems por página (${perPage})`, e => {
    e.stopPropagation(); closeAllMenus(); abrirMenuItemsPerPage();
  });

  addBtn("📎 Exportar CSV", e => {
    e.stopPropagation(); closeAllMenus(); crearMenuCSV(); csvMenu.classList.add("show");
  });

  addBtn("💾 Exportar progreso", e => {
    e.stopPropagation(); closeAllMenus(); exportarProgreso();
  });

  addBtn("📂 Importar progreso", e => {
    e.stopPropagation(); closeAllMenus(); importarProgreso();
  });

  addBtn(`📌 Modo revisión (${modoRevision === "automatico" ? "Automático" : "Bloqueo"})`, e => {
    e.stopPropagation();
    modoRevision = modoRevision === "automatico" ? "bloqueo" : "automatico";
    saveModoRevision();
    generarMenuConfig();
    closeAllMenus();
  });
...
function regenerarMenus() {
  ...
  generarMenuOrden();
  generarMenuConfig();
  generarRevFilterMenu();
  csvMenu?.remove();
  crearMenuCSV();
}

/* ── Modal de filtro genérico (ubicación/época/subcats) ─── */
function aplicarCambioModalFiltro(cat, nuevosValores, totalVisibles) {
  ...
  filtrosState[cat] = nuevosValores;
  currentPage = 1;
  persistState();
  invalidateBaseCache();
...
function crearModalFiltro(titulo, cat, valores, counts) {
  ...
  modal.className = 'temp-floating-menu';
  modal.id = cat + 'Modal';
  ...
  opcionesConDatos.forEach(val => {
    ...
    html += `<label>
      <input type="checkbox" value="${safeVal}" ${filtrosState[cat].includes(val) ? 'checked' : ''} ${totalVisibles === 1 ? 'disabled' : ''}>
      ${val} <span style="color:var(--text-muted);font-size:.6rem">(${counts.get(val)})</span>
    </label>`;
  });
  ...
  const actionsDiv = modal.querySelector(`#${cat}Actions`);
  const mkBtn      = (icon, title) => {
    const b = document.createElement('button');
    b.textContent = icon; b.title = title; b.setAttribute('aria-label', title); return b;
  };

  const sa  = mkBtn('📋', 'Seleccionar todo');
  const da  = mkBtn('🗑️', 'Deseleccionar todo');
  const inv = mkBtn('🔄', 'Invertir selección');
  actionsDiv.append(sa, da, inv);
  ...
  const syncBotones = () => {
    ...
    const n = Array.from(checks).filter(cb => cb.checked).length;
    sa.disabled = n === checks.length;
    da.disabled = n === 0;
  };
  ...
  inv.onclick = () => {
    const ns = []; getChecks().forEach(cb => { cb.checked = !cb.checked; if (cb.checked) ns.push(cb.value); });
    aplicarCambioModalFiltro(cat, ns, totalVisibles); syncBotones();
  };
...
function crearMenuCSV() {
  csvMenu?.remove();
  csvMenu = document.createElement("div");
  csvMenu.id = "csvModal";
  csvMenu.className = "temp-floating-menu";
  renderMenuCSV();
  document.body.appendChild(csvMenu);
...
function renderMenuCSV() {
  ...
  csvMenu.innerHTML = `
    <div class="temp-header">📎 Exportar CSV <span style="font-size:.65rem;color:var(--text-muted)">(nombre siempre incluido)</span>
      <button id="csvCloseBtn" title="Cerrar">✕</button>
    </div>
  ...
  const actionsDiv = document.createElement("div"); actionsDiv.className = "temp-actions";
  ...
  const acceptBtn = document.createElement("button"); acceptBtn.textContent = "✓"; acceptBtn.title = "Exportar";
  acceptBtn.disabled = !selectedCsvFields.length;
  actionsDiv.append(sa, da, inv, acceptBtn);
  csvMenu.appendChild(actionsDiv);

  const sync = () => {
    const n = cg.querySelectorAll("input:checked").length;
    const t = cg.querySelectorAll("input").length;
    sa.disabled = n === t; da.disabled = n === 0; acceptBtn.disabled = !selectedCsvFields.length;
  };
  cg.addEventListener("change", () => {
    selectedCsvFields = [...cg.querySelectorAll("input:checked")].map(c => c.value); sync();
  });
  ...
  inv.onclick = () => {
    selectedCsvFields = [];
    cg.querySelectorAll("input").forEach(c => { if (!c.checked) selectedCsvFields.push(c.value); c.checked = !c.checked; });
    sync();
  };
  acceptBtn.onclick = () => { exportarCSV(); closeTempMenus(); };
...
function exportarCSV() {
  ...
  const ordenadas = ordenarEspecies(aplicarFiltrosCompletos());
  if (!ordenadas.length) return;
  const headers = ['nombre', ...selectedCsvFields];
  ...
  input.onchange = () => {
    ...
    reader.onload = ev => {
      try {
        ...
        if (!Array.isArray(datos.favoritos) || !Array.isArray(datos.revisados))
          throw new Error("Formato inválido");

        closeAllMenus(); closeTempMenus();
        createMenuOverlay(999, () => closeTempMenus());

        const favN = datos.favoritos.length, revN = datos.revisados.length;
        const modal = document.createElement("div");
        modal.className = "temp-floating-menu"; modal.id = "importModal";
        modal.innerHTML = `
          <div class="temp-header">📂 Importar progreso <button id="importCancelBtn" title="Cerrar">✕</button></div>
          <div style="font-size:.8rem;color:var(--text-secondary);line-height:1.8;padding:4px 0">
            <div>⭐ Favoritos en archivo: <strong style="color:var(--accent)">${favN}</strong></div>
            <div>◆ Revisados en archivo: <strong style="color:var(--accent)">${revN}</strong></div>
            <div style="margin-top:8px;font-size:.72rem;color:var(--text-muted)">¿Qué deseas hacer con los datos actuales?</div>
          </div>
        ...
        const btnCombinar  = document.createElement("button"); btnCombinar.title  = "Combinar";  btnCombinar.innerHTML  = "🔀";
        const btnReemplazar = document.createElement("button"); btnReemplazar.title = "Reemplazar"; btnReemplazar.innerHTML = "♻️";
        [btnCombinar, btnReemplazar].forEach(b => {
        ...
        btnCombinar.onclick = () => {
          datos.favoritos.forEach(id => favoritos.add(id));
          datos.revisados.forEach(id => revisados.add(id));
          saveFavoritos(); saveRevisados(); closeTempMenus(); invalidateBaseCache(); render();
          showTooltip(`Combinado: +${favN} fav, +${revN} rev`, window.innerWidth/2, 80, 2500);
        };
        btnReemplazar.onclick = () => {
          if (!confirm(`¿Reemplazar datos actuales (${favoritos.size} fav, ${revisados.size} rev) con los del archivo?`)) return;
          favoritos = new Set(datos.favoritos); revisados = new Set(datos.revisados);
          saveFavoritos(); saveRevisados(); closeTempMenus(); invalidateBaseCache(); render();
          showTooltip(`Reemplazado: ${favN} fav, ${revN} rev`, window.innerWidth/2, 80, 2500);
        };

        actDiv.append(btnCombinar, btnReemplazar);
      } catch (err) {
...
function abrirMenuItemsPerPage() {
  closeAllMenus(); closeTempMenus();
  createMenuOverlay(999, () => closeTempMenus());
  const valores   = [12, 24, 48, 96];
  const sliderIdx = Math.max(0, valores.indexOf(perPage));
  let tempPP      = perPage;
  ...
  menu.innerHTML = `
    ...
    <div class="slider-container">
      <input type="range" id="itemsSlider" min="0" max="3" step="1" value="${sliderIdx}">
      <div class="slider-labels"><span>12</span><span>24</span><span>48</span><span>96</span></div>
    </div>
  ...
  const slider    = menu.querySelector("#itemsSlider");
  const valSpan   = menu.querySelector("#itemsTempVal");
  const acceptBtn = menu.querySelector("#itemsAcceptBtn");
  slider.addEventListener("input", () => { tempPP = valores[+slider.value]; valSpan.textContent = tempPP; acceptBtn.disabled = tempPP === perPage; });
  acceptBtn.onclick = () => { if (!acceptBtn.disabled) { perPage = tempPP; currentPage = 1; render(); persistState(); generarMenuConfig(); closeTempMenus(); } };
...
function abrirMenuAyuda() {
  ...
  menu.innerHTML = `
    <div class="temp-header">📖 Ayuda <button id="helpCloseBtn" title="Cerrar">✕</button></div>
    <div class="help-content">
      <p><strong>🔮 Morfología</strong> — Forma física del ser</p>
      <p><strong>🌙 Tipo</strong> — Mitológico, Criptido o Difuso</p>
      <p><strong>💧 Elemento</strong> — Aéreo, Acuático, Terrestre…</p>
      <p><strong>👁️ Avistamiento</strong> — Nivel de evidencia</p>
      ...
      <p><strong>✅ Revisado</strong> — Filtrar por estado</p>
      <p><strong>⭐ Favoritos</strong> — Solo tus favoritos</p>
      <p><strong>🧹 Limpiar</strong> — Quita filtros activos</p>
      <p><strong>📊 Ordenar</strong> — Cambia el criterio</p>
      <p><strong>⚙️ Config</strong> — Ítems, CSV, modo revisión</p>
      <p style="margin-top:8px;font-size:.68rem;color:var(--text-muted)">
        Teclado en modal: ← → navegar · F favorito · R revisado · L abrir enlace · Esc cerrar
      </p>
    </div>`;
  document.body.appendChild(menu); menu.classList.add('show');
...
function abrirVentanaAyudaEsquina() {
  ...
  menu.innerHTML = `
    <div class="temp-header">AYUDA <button id="ayudaEsquinaCerrarBtn" title="Cerrar">✕</button></div>
    <div class="help-content">
      <p>Los seres místicos merecen un entorno digno y compañía. Si ves errores o tienes sugerencias, escríbeme. También puedes aportar nuevas criaturas con nombre, información o enlaces. Gracias por ayudar.</p>
...
headerCornerImg.addEventListener('click', e => { e.stopPropagation(); abrirVentanaAyudaEsquina(); });
